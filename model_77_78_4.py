...
class Learner():
    ...
    def prepare_data(self, data, length, flag_random):
        n = data.shape[0]
        pdata = []
        ...
        for i in range(n):
            ...
            if (flag_random):
                ...
                for j in range(num_repeat):
                    Start = random.randint(0, m - length)
                    aa = asd[Start : Start + length]
                    pdata.append(aa)
            else:
                for j in range(num_repeat):
    ...
    def learn(self, fol):
        tbCallBack = keras.callbacks.TensorBoard(log_dir='../Graph_test2_fold%d' % fol, \
                            histogram_freq=0, write_graph=True, write_images=True)
        checkpointer = ModelCheckpoint(filepath='/data/tmpsrt1/log_new/weights_test2_fold%d.{epoch:02d}.hdf5' % fol, \
